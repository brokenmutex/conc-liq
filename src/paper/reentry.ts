import {paperMarket,namedBalances,marketValue} from './market.js';
...
export function closedPaperCash(session: PaperSessionRow): string {
  ...
  const cashInventory=namedBalances(paperMarket(session.policy),BigInt(p.idle0),BigInt(p.idle1));
  assert(p.liquidity === "0" && cashInventory.rwa === 0n && p.fee0 === "0" && p.fee1 === "0" && s.exitReserveQuote === "0",
    "Paper continuation has residual inventory or costs");
  const cash = cashInventory.quote - BigInt(s.costsPaidQuote);
...
export async function readPaperChain(client: PoolClient, latest: PaperSessionRow): Promise<PaperSessionRow[]> {
  ...
  for (;;) {
    ...
    assert.deepEqual(paperMarket(parent.policy),paperMarket(row.policy),"Paper continuation market changed");
    row = parent;
  }
  return chain;
}
...
export function continuationPolicy(parent: PaperSessionRow, policy: TransactionPaperPolicy): TransactionPaperPolicy {
  assert.deepEqual(paperMarket(parent.policy),paperMarket(policy),"Paper continuation market changed");
...
export function paperCampaignSummary(chain: readonly PaperSessionRow[]) {
  ...
  const hold = baseline && mark ? String(marketValue(paperMarket(first.policy),BigInt(mark.sqrtPriceX96),BigInt(baseline.hold0),BigInt(baseline.hold1)) - BigInt(first.state.execution?.holdGasQuote ?? "0")) : null;
