...
export async function upgradePaperRuntime(db:PoolClient,stream:string,id:string,fromBuild:string,to:RuntimeIdentity) {
 ...
 try {
  ...
  assert(to.configHash===row.runtime_identity.configHash&&to.nodeVersion===row.runtime_identity.nodeVersion&&to.buildId!==fromBuild,'Only a code upgrade with identical configuration is allowed');
  ...
  const transition={from:row.runtime_identity,to,throughRunId:boundary.run,throughObservationId:boundary.observation,at:boundary.at.toISOString(),
  ...
  await db.query('UPDATE paper_sessions SET runtime_identity=$2,state=$3,updated_at=clock_timestamp() WHERE id=$1',[id,JSON.stringify(to),JSON.stringify(state)]);
  await readPaperChain(db,{...row,state,runtime_identity:to});
