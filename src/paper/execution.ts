...
export async function createPaperExecutionContext(fork: PaperFork, policy: PaperExecutionPolicy, onTransaction?: (tx: PaperTransaction) => void, account:Address=PAPER_ACCOUNT) {
  ...
  async function send(action: string, to: Address, calldata: Hex) {
    ...
    try { result = await simulatePaperTransaction(fork, { action, to, calldata },account); }
...
export async function simulatePaperRoundTrip(fork: PaperFork, policy: PaperExecutionPolicy, onTransaction?: (tx: PaperTransaction) => void,
  ...
  assert(BigInt(before.quote)>=BigInt(policy.budgetQuote),'Insufficient operator USDG');assert.equal(before.rwa, "0");
  ...
  for (const token of [USDG, NVDA]) for (const spender of [PAPER_ROUTER, NONFUNGIBLE_POSITION_MANAGER]) {
    ...
    assert.equal(await local.readContract({ address: token, abi: paperTokenAbi, functionName: "allowance", args: [account, spender] }), 0n);
  }
