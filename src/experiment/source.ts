...
export class ExperimentSource {
 ...
 async coverage(block:string,target:string){const r=(await this.db.query(`SELECT COALESCE(i.covered_through_block,i.last_scanned_block)::text AS last_scanned_block,r.complete_through_block::text,i.target_set_hash,r.target_set_hash AS replay_target FROM indexer_cursors i JOIN v3_replay_cursors r USING(stream_key) WHERE i.stream_key=$1 AND i.chain_id=4663 AND r.chain_id=4663`,[STREAM])).rows[0];assert(r&&BigInt(r.last_scanned_block)>=BigInt(block)&&BigInt(r.complete_through_block)>=BigInt(block)&&r.target_set_hash===target&&r.replay_target===target,'Event coverage unavailable');}
 async costs(){
