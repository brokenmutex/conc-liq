...
import pg from 'pg';
import { evaluatePaperReference } from '../paper/reference.js';
...
export class ExperimentSource {
 ...
 frame(row:Record<string,any>,health:readonly {id:string;snapshot:RpcHealthEvaluation}[],events:ExperimentEvent[]):ExperimentFrame{
  const sourceAt=new Date(row.source_at).toISOString(),observedAt=new Date(new Date(row.observed_at).getTime()+30000).toISOString();
  ...
  return {id:row.id,block:row.block,hash:row.hash,sourceAt,observedAt,targetSetHash:row.target_set_hash,price:row.price,tick:row.tick,liquidity:row.liquidity,global0:row.global0,global1:row.global1,
 ...
 async historyValid(ids:string[]){const r=await this.db.query(`SELECT count(*)::int AS n FROM v3_strategy_checkpoint_runs c JOIN risk_snapshot_canonicality v ON v.risk_run_id=c.risk_run_id WHERE c.id=ANY($1::bigint[]) AND v.canonical IS TRUE AND v.block_number=c.block_number AND LOWER(v.expected_hash)=LOWER(c.block_hash) AND LOWER(v.observed_hash)=LOWER(c.block_hash)`,[ids]);return r.rows[0].n===ids.length;}
}
