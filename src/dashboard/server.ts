  ...
  return createServer(async (request, response) => {
    ...
    try {
      ...
      if(pathname === "/api/positions" || pathname.startsWith("/api/positions/")) {
        ...
        if((id!==undefined&&!/^(paper-[1-9]\d*|paper-adaptive-[a-z0-9.]+|(paper|live)-dep-[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}|live-(rk-)?[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})$/.test(id))||![1,6,24,168].includes(hours)) {
