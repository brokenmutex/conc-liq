...
function adaptiveSummary(state:any,asset:any){
 ...
 return {id:`paper-adaptive-${String(asset.symbol).toLowerCase()}`,label:`A-${Math.round((state.config.forecast?.lookbackMs??3600000)/60000)}m${state.config.residualRange?'+R':''}`,mode:'paper',asset:asset.symbol,quote:'USDG',fee:market.fee,quoteIsToken0:marketTokens(market).quoteIsToken0,
  sessionIds:[],hasLiquidity:!!p,status,history:invalid,initialQuote:asset.budgetQuote??state.config.budgetQuote,navQuote:invalid?null:String(nav),holdQuote:invalid||hold===null?null:String(hold),reference:invalid?null:(asset.reference??null),feesQuote:invalid?null:String(fees),gasQuote:invalid?null:String(model.gas),
