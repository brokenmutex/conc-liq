// Offline asset-universe extension; original policy and fee code remain frozen.
...
const [symbol,budgetText,source='data/adaptive-lp-universe-study-2026-09-13',root=source+'/runs']=process.argv.slice(2);
...
assert.equal(hash(readFileSync('notes/adaptive-lp-universe-study-2026-09-13/plan.json')),complete.planSha256);
assert.equal(hash(readFileSync(source+'/capture.json')),complete.captureSha256);assert(complete.canonicalEndsVerified&&complete.indexedDbLogsExactMatch);
const amendmentPath='notes/adaptive-lp-universe-study-2026-09-13/evidence-amendment.json',amendment=read(amendmentPath);assert.equal(amendment.planSha256,complete.planSha256);
const prior=read('notes/adaptive-lp-study-2026-09-13/manifest.json'),inclusion=read(source+'/inclusion.json');assert.equal(inclusion.captureSha256,complete.captureSha256);
for(const [p,h] of Object.entries(prior.code))assert.equal(hash(readFileSync(p)),h,'Frozen original replay code changed: '+p);
assert(plan.symbols.includes(symbol)&&plan.budgetsQuote.includes(budgetText));const budget=BigInt(budgetText),asset={...meta.assets.find(a=>a.symbol===symbol),...inclusion.included.find(a=>a.symbol===symbol)};assert(inclusion.included.some(a=>a.symbol===symbol),'Asset evidence incomplete');
const out=root+'/'+symbol+'-'+budgetText;mkdirSync(out,{recursive:true});assert(!existsSync(out+'/manifest.json'),'Output exists');
writeFileSync(out+'/manifest.json',json({planSha256:complete.planSha256,captureSha256:complete.captureSha256,captureCompleteSha256:hash(readFileSync(source+'/completed.json')),source,symbol,budgetQuote:budgetText,availabilityAmendmentSha256:hash(readFileSync(amendmentPath)),code:prior.code,runnerSha256:hash(readFileSync('scripts/adaptive-lp-universe-study.mjs')),inclusionSha256:hash(readFileSync(source+'/inclusion.json')),createdAt:new Date().toISOString()})+'\n');
assert.equal(asset.market.pool.toLowerCase(),asset.pool.toLowerCase());assert.equal(asset.market.fee,500);for(const [p,h] of Object.entries(asset.evidenceHashes))assert.equal(hash(readFileSync(p)),h);
// Controls are reused only after independent raw-event, seed, end and cost identity checks.
if(asset.retainedRunRoot){
 const old=asset.retainedRunRoot+'/'+symbol+'-'+budgetText,oldManifest=read(old+'/manifest.json'),done=read(old+'/completed.json'),raw=readFileSync(old+'/results.json');
 assert.equal(hash(raw),done.resultsSha256);assert.equal(hash(readFileSync('scripts/adaptive-lp-long-study.mjs')),oldManifest.runnerSha256);
 assert(asset.priorControlIdentityVerified);assert.equal(oldManifest.captureSha256,inclusion.retainedCaptureSha256);
 writeFileSync(out+'/results.json',raw);writeFileSync(out+'/reuse.json',json({source:old,resultsSha256:hash(raw),manifestSha256:hash(readFileSync(old+'/manifest.json')),identity:'Canonical events, seed, ending state, cost bundle and decision parameters match prior verified control'})+'\n');
 writeFileSync(out+'/completed.json',json({completedAt:new Date().toISOString(),rows:18,resultsSha256:hash(raw),reused:true})+'\n');console.log(json({stage:'control_reused',symbol,budgetQuote:budgetText}));process.exit(0);
}
...
let pending=[];
for(const page of asset.pages){const raw=readFileSync(source+'/'+page.file);assert.equal(hash(raw),page.sha256);for(const e of JSON.parse(gunzipSync(raw)).events){if(e.symbol!==symbol||Number(e.block)<=asset.seedBlock)continue;if(pending.length&&pending[0].block!==e.block){await applyBlock(pending);pending=[];}pending.push(e);}}
if(pending.length)await applyBlock(pending);book.verify(asset.after);assert(hold&&lastSource&&firstAt!==null);assert.equal(firstAt,asset.availability.at);assert.equal(book.protocol0,asset.after.protocol0);assert.equal(book.protocol1,asset.after.protocol1);
const results=[];
for(const s of entries){const result={symbol,budgetQuote:budgetText,requestedFromAt:start,preBenchmarkCashWaitMs:firstAt-start,fullWindowMs:lastSource.at-start,scenario:s.scenario,...s.model.summary(lastSource,hold)};
 ...
 if(result.terminalCashQuote!==null||result.holdTerminalCashQuote!==null){const cash=result.terminalCashQuote===null?0n:BigInt(result.terminalCashQuote)-s.nav,h=result.holdTerminalCashQuote===null?0n:BigInt(result.holdTerminalCashQuote)-s.holdValue,gas=result.terminalCashQuote===null?0n:BigInt(result.terminalExitCostQuote);add(bucket(s.weekly,s.lastWeek,lastSource.at),lastSource.at,cash,h,0n,gas,0);add(bucket(s.regimes,'terminal_liquidation',lastSource.at),lastSource.at,cash,h,0n,gas,0);}
