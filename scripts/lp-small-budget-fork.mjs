...
const [envPath,dir,symbolList='AAPL,QQQ,NVDA',halfWidthText='80']=process.argv.slice(2);assert(envPath&&dir);
...
const halfWidth=Number(halfWidthText);assert(Number.isInteger(halfWidth)&&halfWidth>0&&halfWidth%10===0);base.halfWidthSpacings=halfWidth/10;base.budgetQuote='240000000';
...
try{
 ...
 for(const symbol of symbolList.split(',')){
  assert(!existsSync(dir+`/fork-${symbol}.json`),'Fork evidence already exists; use a new output directory');
  ...
  try{
   ...
   const outside={...inventory,tickLower:range.tickUpper+20,tickUpper:range.tickUpper+60,liquidity:'1000',...Object.fromEntries(Object.entries(canonicalBalances(market,240000000n,0n)).map(([k,v])=>[k==='amount0'?'idle0':'idle1',String(v)])),fee0:'3',fee1:'5'};
   ...
   result.passed=Object.values(result.stages).every(s=>s.passed);result.limitations=['240 USDG synthetic inventory, ±80 ticks; pinned fork costs, not live receipts; uninitialized canonical boundaries are permitted for virtual positions','Recenter uses deliberately constructed one-sided inventory'];
