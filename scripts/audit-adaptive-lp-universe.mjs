...
const root=process.argv[2]??'data/adaptive-lp-universe-study-2026-09-13',read=p=>JSON.parse(readFileSync(p)),hash=x=>createHash('sha256').update(x).digest('hex');
...
assert(only===undefined||plan.symbols.includes(only));const inclusion=read(root+'/inclusion.json');const assets=meta.assets.filter(a=>inclusion.included.some(x=>x.symbol===a.symbol)&&(only===undefined||a.symbol===only)).map(a=>({...a,...inclusion.included.find(x=>x.symbol===a.symbol)}));
...
function applyBlock(events){const symbol=events[0].symbol,book=books.get(symbol),ss=states.get(symbol),at=events[0].at;
 ...
 for(const s of ss){if(at<s.r.fromAt||BigInt(events[0].block)<BigInt(s.asset.availability.block))continue;const oldNav=s.nav,oldHold=s.holdValue,value=(a,b)=>marketValue(s.asset.market,m.price,a,b),q0=marketTokens(s.asset.market).quoteIsToken0;
  if(!s.active){assert.equal(events[0].block,s.asset.availability.block);assert.equal(at,s.r.fromAt);s.active=true;const budget=BigInt(s.r.budgetQuote),buy=historicalSwapQuote(m,budget/2n,q0?0:1,plan.slippageBps);assert(buy.fullyFilled&&buy.passesSlippage);s.hold={amount0:q0?budget-budget/2n:buy.amountOut,amount1:q0?buy.amountOut:budget-budget/2n};}
...
for(const page of (assets.length===1?assets[0].pages:complete.pages)){const raw=readFileSync(root+'/'+page.file);assert.equal(hash(raw),page.sha256);for(const e of JSON.parse(gunzipSync(raw)).events){if(!pending.has(e.symbol)||Number(e.block)<=assets.find(a=>a.symbol===e.symbol).seedBlock)continue;let batch=pending.get(e.symbol);if(batch.length&&batch[0].block!==e.block){applyBlock(batch);batch=[];pending.set(e.symbol,batch);}batch.push(e);}}
...
for(const asset of assets){const book=books.get(asset.symbol);book.verify(asset.after);for(const s of states.get(asset.symbol)){assert(s.active);assert.equal(String(s.nav),s.r.markedNavQuote);assert.equal(String(s.gas),s.r.gasPaidQuote);assert.equal(String(s.earned0),s.r.fees0);assert.equal(String(s.earned1),s.r.fees1);assert.equal(s.applied,s.r.actions.length);assert.equal(s.invalid,s.r.invalid!==null);if(!s.invalid)assert.equal(s.inRangeMs,s.r.holdingMs-s.r.outsideMs);assert(s.above100Ms<=s.above10Ms&&s.above10Ms<=s.inRangeMs);for(const token of [0,1])assert(s.above100[token]<=s.above10[token]&&s.above10[token]<=(token===0?s.earned0:s.earned1));
 const m=book.source(),q0=marketTokens(asset.market).quoteIsToken0,principal=s.p?principalAmounts({...s.p,sqrtPriceX96:m.price}):{amount0:0n,amount1:0n},balances={amount0:s.cash0+principal.amount0+(s.p?s.p.fee0/Q:0n),amount1:s.cash1+principal.amount1+(s.p?s.p.fee1/Q:0n)},multiplier=BigInt(plan.scenarios.find(x=>x.name===s.r.scenario).gasMultiplier);
 const close=(b,gas,exit)=>{const risky=q0?b.amount1:b.amount0,cash=q0?b.amount0:b.amount1;try{const q=risky?historicalSwapQuote(m,risky,q0?1:0,plan.slippageBps):null;if(q&&(!q.fullyFilled||!q.passesSlippage))return null;return String(cash+(q?.amountOut??0n)-gas-exit);}catch{return null;}};
 const exitCost=(s.p||(q0?balances.amount1:balances.amount0)>0n)?BigInt(asset.costs.exit)*multiplier:0n;
 assert.equal(String(exitCost),s.r.terminalExitCostQuote);assert.equal(close(balances,s.gas,exitCost),s.r.terminalCashQuote,'Reconstructed strategy liquidation differs');assert.equal(close(s.hold,BigInt(asset.costs.hold)*multiplier,BigInt(asset.costs.holdExit)*multiplier),s.r.holdTerminalCashQuote,'Reconstructed passive liquidation differs');
 if(s.r.terminalCashQuote!==null||s.r.holdTerminalCashQuote!==null){const n=s.r.terminalCashQuote===null?0n:BigInt(s.r.terminalCashQuote)-s.nav,h=s.r.holdTerminalCashQuote===null?0n:BigInt(s.r.holdTerminalCashQuote)-s.holdValue;add(s.weekly,s.lastWeek,n,h);add(s.regimes,'terminal_liquidation',n,h);}
