...
import {OffHoursCap,CAP_HOURS} from '../src/research/offhours-cap.ts';
...
assert.deepEqual(plan.caps,[600000,700000,800000]);assert.deepEqual(plan.tradingHours,CAP_HOURS);
const health=new Map(data.health.map(h=>[h.id,h])),market=new ExperimentMarket(data.seed),windows=[];
let window=null,last=null;
const profiles=plan.profiles.map(p=>({...p,entry:BigInt(p.entry),exit:BigInt(p.exit),buy:BigInt(p.buy)}));
function finish(frame,partialEnd=false){
 ...
 for(const profile of profiles){const rs=results.filter(r=>r.profile===profile.id),first=rs.map(r=>r.actions.find(a=>a.kind==='entry')?.checkpoint);assert(first.every(x=>x===first[0]),'Caps differ before initial entry');
  const baseline=rs.find(r=>r.cap===600000);
  const available=!window.partialStart&&!partialEnd&&rs.every(r=>!r.invalid&&!r.positionOpen);
  for(const r of rs){r.available=available;r.delta=available?r.nav-baseline.nav:null;}}
...
for(const f of data.frames){
 ...
 if(!window&&schedule.allowed){
  window={startAt:f.sourceAt,excludedAt:schedule.excludedAt,partialStart:!last||equityHours(last.observedAt).allowed,
   models:profiles.flatMap(p=>plan.caps.map(cap=>({profile:p.id,model:new OffHoursCap(BigInt(plan.budgetQuote),cap,p)})))};
 }
 if(window){
...
const aggregates=[];
for(const profile of profiles)for(const cap of plan.caps){
 const rows=windows.map(w=>w.results.find(r=>r.profile===profile.id&&r.cap===cap)),valid=rows.filter(r=>r.available);
 const sum=k=>valid.reduce((n,r)=>n+Number(r[k]),0);
 aggregates.push({profile:profile.id,cap,windows:rows.length,available:valid.length,traded:valid.filter(r=>r.entries).length,
...
const out={computedAt:new Date().toISOString(),captureSha256:digest(raw),planSha256:digest(fs.readFileSync(planPath)),manifest:data.manifest,plan,windows,aggregates,
 method:{scope:'Independent allowed trading windows with common starting cash, compounding after each full cash exit and 600 second cooldown; caps are the sole policy difference',
 ...
 costs:'Saved fork entry/exit estimates reused across sizes and dates; 2x gas plus 50 percent modeled fees sensitivity. Hypothetical entry failures incur no transaction gas because checks precede mint; partial transaction failures are not modeled.',
