...
exact post-entry inventory and shared entry cost. It also carries source-bound
independent reference marks into a separate NAV, passive value and alpha series;
an unavailable reference produces unavailable values rather than a pool-price
substitute. Older sidecar marks that reported the initial cash budget as holding
NAV are excluded from alpha. The currently deployed sealed paper release
predates these changes, so current reported return still cannot be called LP
alpha.
...
- Deploy the passive benchmark and independent-reference series through the
  guarded runtime migration, then collect provenance-bearing marks.
