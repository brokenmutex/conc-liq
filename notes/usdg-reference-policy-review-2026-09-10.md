...
## Release and queued activation

At **2026-09-10 11:43:23 UTC / 14:43:23 Vilnius**, session **52 remained open** on its original policy and build. Source commit `1121e3fa52df9d4c5654f9a8f7b66a0f14da4db7` is built and verified as sealed release `81087b0e63143aa12916aadf70e7e9e18162c1936e63e300ba90e5d21b5950b1`. Typechecking, the full unit suite, the isolated reference gate, the boundary/reentry lifecycle, and compiled-release smoke checks passed. The compiled new reference evaluator also accepts the saved #37 four-second overrun.

The new release is **queued, not yet active for session 52**. A temporary `ExecStartPost` callback starts `conc-liq-usdg-grace-activation.service` after each ordinary paper invocation. Three observed callbacks returned `waiting_for_cash_exit`, leaving the open position, timer and runtime unchanged. The reviewed plan pins parent 52, its policy hash, previous/new builds, the private configuration hash and both service files. Only the normal cash exit allows the handoff. Paper timer remains active/enabled; the experiment service remains inactive and its frozen ledger hash unchanged.

[Verification snapshot](usdg-grace-activation-2026-09-10.json) records the valid 5–52 funding/evidence chain and deployment state. The plan is `data/usdg-grace-2026-09-10/plan.json`; successful automatic activation writes `data/usdg-grace-2026-09-10/activation.json` and the closed parent evidence. To inspect the later boundary, read that result and `journalctl -u conc-liq-usdg-grace-activation.service`; do not infer activation from the prepared build alone.

### Handoff race correction

Session 52 naturally exited on its inventory threshold at 11:48:49 UTC. The first callback dispatched activation asynchronously, allowing the old timer to create session 53 while activation verified the release and waited for the worker to become idle. The parent-ID assertion rejected the handoff (`53 != 52`); no session policy or historical funding was rewritten. The old worker/timer continued normally. The failure journal and original plan are retained as `data/usdg-grace-2026-09-10/missed-boundary.log` and `plan-parent52.json`.

The callback now runs a short synchronous `--arm` step inside the old worker's `ExecStartPost`. It stops the timer while that worker is still in its post-start phase, then dispatches the separate activation service. Release verification and linked-session creation happen after the old invocation finishes. The isolated handoff regression executes the actual helper with mocked database/systemd and temporary files: it proves no service actions while open, timer-stop before dispatch, and activation through the ordinary linked-session command.

The plan now targets **session 53's normal cash exit**. Session 53 retains its original immutable policy; there is no forced exit to repair deployment. The new USDG grace is still pending until the activation result exists. The separate [infrastructure exit review](lp-infrastructure-exit-plan-2026-09-10.md) makes no additional runtime policy change.
