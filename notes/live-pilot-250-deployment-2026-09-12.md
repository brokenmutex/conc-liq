...
The separate [configuration](../config/live-pilot-nvda-250.json) has `broadcastEnabled: false`, no operator and no signer. Gas funding remains unset until the wallet and launch budget are selected. A capital limit constrains initial funding, not the value of an appreciating position.
...
4. **Wallet-specific launch rehearsal and deployment.** Public operator address and signing setup are still missing. Read actual USDG/ETH balances, allowance inventory, existing NFTs and pending nonces; prove sender identity and rehearse the exact funded-wallet path. Reserve enough ETH for entry and a fully costed withdrawal/sell/revoke recovery, using fresh buffered estimates. Install a separate live service and receipt-backed dashboard panel; keep the paper release and scheduled comparison checkpoints intact.
