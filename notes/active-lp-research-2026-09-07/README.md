...
The user clarified the current width focus as [±10–±50 raw ticks around the price](../active-lp-tick-half-width-screen-2026-09-07.md), giving total widths of 20, 40, 60, 80 and 100 ticks. The `ticks` command uses `tick-half-width-selection.json` and writes `tick-half-width-screen-v1.json`, with explicit per-side and total width fields. The earlier `tick-width-selection.json` and compact run manifest retain the superseded total-width interpretation; its full deterministic output was removed after a verified file-only replay and remains recoverable through `research/manifests/active-lp-tick-width-2026-09-07/pruned-artifacts.json`. Percent-width artifacts below are also historical baselines.
...
- `size-sweep-v1.json.manifest.json` and `size-sweep-v1.json`: fingerprints, verified coverage, all sizes/policies/widths, initial and subsequent hypothetical placements, active minute shares and overlapping swap-segment shares.
