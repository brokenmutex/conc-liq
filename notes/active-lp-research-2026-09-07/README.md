...
The user clarified the current width focus as [±10–±50 raw ticks around the price](../active-lp-tick-half-width-screen-2026-09-07.md), giving total widths of 20, 40, 60, 80 and 100 ticks. The `ticks` command uses `tick-half-width-selection.json` and writes `tick-half-width-screen-v1.json`, with explicit per-side and total width fields. The earlier `tick-width-selection.json` / `tick-width-screen-v1.json` artifacts retain the superseded total-width interpretation. Percent-width artifacts below are also historical baselines.
