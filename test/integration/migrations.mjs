...
try {
 ...
 assert.deepEqual(await migrateDatabase(client),[1,2,3,4]);
 ...
 assert.deepEqual(await migrateDatabase(client),[4]);
 await assertSchemaReady(client);
 ...
 assert.deepEqual(await migrateDatabase(client,{baseline:true}),[1,2,3,4]);
 ...
 await assert.rejects(assertSchemaReady(client),/schema incompatible/);
 console.log(JSON.stringify({passed:['fresh migration','v3-to-v4 isolated upgrade','idempotency','read-only readiness','checksum rejection','verified existing baseline','exact production legacy constraints accepted','legacy rows unchanged','worker starts without DDL','application locks do not block readiness','schema drift rejected atomically','search-path isolation','monotone coverage cursor']}));
}finally{
