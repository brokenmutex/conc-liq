...
describe('forward experiment persistence',()=>{
 ...
 it('does not retroactively fill checkpoints missed by a stopped worker',async t=>{
  ...
  const fake={checkpoints:async()=>inputs.map(f=>({id:f.id,block:f.block,source_at:f.sourceAt,observed_at:new Date(Date.parse(f.observedAt)-30000).toISOString(),target_set_hash:f.targetSetHash})),seed:async()=>seed(),health:async()=>[],events:async()=>[],coverage:async()=>{},costs:async()=>({costs}),historyValid:async()=>true,frame:(r:{id:string})=>inputs.find(f=>f.id===r.id)!} as unknown as import('../src/experiment/source.js').ExperimentSource;
  try{await startForward(fake,[portfolio().candidate,{...portfolio().candidate,id:'other'}],costs,identity,path);inputs.push(frame(1060000));t.mock.timers.tick(300000);const state=await tickForward(fake,path,identity);assert.equal(state.status,'invalid');assert.equal(state.reason,'missed_forward_decision');assert.equal(state.states[0]!.entries,0);}finally{await rm(dir,{recursive:true,force:true});t.mock.timers.reset();}
 });
});
